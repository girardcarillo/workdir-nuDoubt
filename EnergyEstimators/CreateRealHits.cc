...
vector<realHit> CreateRealHits(TTree *tree, int targetEventNumber = 0, bool Include_FiberSiPM_efficiency = 1){
  ...
  for (const auto& entry : hitsMap){
	...
	sumTime += hit.time;
      ...
      // Deteriorate light yield using detection efficiency
      ...
      if (Include_FiberSiPM_efficiency) numHits = numHits*gRandom->Gaus(1/18.,1/18.); // if we want to account (artificially) for the efficiency of fibers+SiPMs, we degrade this number. Final number: 500 PE/MeV should be collected (sims: 9000 PE?MeV) => factor 18.
